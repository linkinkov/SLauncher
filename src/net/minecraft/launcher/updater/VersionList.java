...
public abstract class VersionList {
	...
	public CompleteVersion getCompleteVersion(Version version)
        ...
        if (version.getID().toLowerCase().contains("optifine") || version.getID().toLowerCase().contains("forge")) {
            complete = gson.fromJson(getUrl("versions/" + version.getID() + ".json"), CompleteVersion.class);
        } else {
            complete = gson.fromJson(getUrl("versions/"+ version.getID() +"/"+ version.getID() +".json"), CompleteVersion.class);
        }
	...
	public static class RawVersionList {
		...
		Map<ReleaseType, String> latest = new EnumMap<ReleaseType, String>(ReleaseType.class);
