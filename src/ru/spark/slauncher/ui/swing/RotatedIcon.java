 ...
 * @author Rob Camick
 * @see <a
 *      href="http://tips4java.wordpress.com/2009/04/06/rotated-icon/">Article
 *      at Tips4java.com</a>
 */
public class RotatedIcon implements Icon {
	...
	public int getIconWidth() {
		if (rotate == Rotate.ABOUT_CENTER) {
			...
			int width = (int) Math.floor(icon.getIconWidth() * cos
	...
	public int getIconHeight() {
		if (rotate == Rotate.ABOUT_CENTER) {
			...
			double sin = Math.abs(Math.sin(radians));
			double cos = Math.abs(Math.cos(radians));
			int height = (int) Math.floor(icon.getIconHeight() * cos
