...
import ru.spark.slauncher.ui.explorer.ImageFileFilter;
...
public class SlideBackgroundThread extends LoopedThread {
	...
	public synchronized void refreshSlide(boolean animate) {
		String path = SLauncher.getInstance().getSettings()
				.get("gui.background");
	...
	public synchronized void setSlide(Slide slide, boolean animate) {
		...
		if(image == null) {
			log("Default image is NULL. Check accessibility to the JAR file of SLauncher.");
			return;
		}
