...
import ru.spark.util.async.AsyncObject;
import ru.spark.util.async.AsyncObjectContainer;
import ru.spark.util.async.AsyncObjectGotErrorException;
...
public class VersionManager extends InterruptibleComponent {
	...
	public List<VersionSyncInfo> getVersions(boolean includeLatest) {
		return getVersions(SLauncher.getInstance() == null ? null : SLauncher
	...
	public List<VersionSyncInfo> getInstalledVersions() {
		return getInstalledVersions(SLauncher.getInstance() == null ? null
				: SLauncher.getInstance().getSettings().getVersionFilter());
	}
