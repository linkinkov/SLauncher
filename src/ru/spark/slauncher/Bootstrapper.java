...
import ru.spark.slauncher.ui.LoadingFrame;
...
public final class Bootstrapper {
	...
	public static JavaProcessLauncher createLauncher(String[] args, boolean loadAdditionalArgs) {
		...
		if(loadAdditionalArgs) {
			File argsFile = new File(DIRECTORY, "slauncher.args");
	...
	private static final String MAIN_CLASS = "ru.spark.slauncher.SLauncher";
	...
	public void start() throws IOException {
		...
		frame.setTitle("SLauncher "+ SLauncher.getVersion());
	...
	private class BootstrapperListener implements JavaProcessListener {
		...
		public void onJavaProcessEnded(JavaProcess jp) {
			...
			if((exit = jp.getExitCode()) != 0) {
				Alert.showError(
						"Error starting SLauncher"
						, "SLauncher application was closed with illegal exit code ("+ exit+"). See console:"
						, buffer.toString());
			}
