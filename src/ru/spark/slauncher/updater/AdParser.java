...
public final class AdParser {
    ...
    public static AdList adlist = new AdList("ru_RU");
	...
	public static AdMap parse(JsonObject object) {
		if(object == null)
			throw new NullPointerException("JsonObject is null!");

		String locale = SLauncher.getInstance().getLang().getSelected().toString();
		AdMap map = new AdMap();
			try {
				JsonArray array = object.get("ads").getAsJsonObject().get(locale).getAsJsonArray();
				if (array.size() < 1) {
					return map;
				}
				Ad ad = new Ad(array.get(0).getAsJsonObject().get("name").getAsString(),
                        array.get(0).getAsJsonObject().get("chance").getAsInt(),
                        array.get(0).getAsJsonObject().get("content").getAsString(),
						new int[] {array.get(0).getAsJsonObject().get("size1").getAsInt(), array.get(0).getAsJsonObject().get("size2").getAsInt()},
                        array.get(0).getAsJsonObject().get("image").getAsString());
				adlist.add(ad);
                log("Parsed ad: " + ad.getName());
			} catch(RuntimeException rE) {
				log("Cannot parse key", object.toString(), rE);
			}
