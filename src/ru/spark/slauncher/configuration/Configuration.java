...
import ru.spark.slauncher.minecraft.launcher.MinecraftLauncher.ConsoleVisibility;
...
public class Configuration extends SimpleConfiguration {
	...
	public static Configuration createConfiguration(OptionSet set) throws IOException {
		...
		if (path == null) {
			...
			file = FileUtil.getNeighborFile("slauncher.cfg");

			if (!file.isFile())
				file = FileUtil.getNeighborFile("slauncher.properties");

			if(!file.isFile())
				file = MinecraftUtil.getSystemRelatedDirectory(SLauncher.getSettingsFile());
	...
	private void init(OptionSet set) {
		...
		this.comments = " SLauncher configuration file" + '\n' + " Created in "
				+ SLauncher.getBrand() + " " + SLauncher.getVersion();
